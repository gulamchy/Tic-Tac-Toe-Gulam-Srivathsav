...
app.post("/games/:gameId", wrapAsync(async (req, res) => {
  ...
  const stopProcessing = await handlePostMoveNotification({ game, mover, opponent });
  if (stopProcessing) {
    return res.json({ message: 'Game over. The winner is emailed'});
  }
  res.json(game);
}));
